...
function cursorUp(n: number) { stderr.write(`\x1b[${n}A`); }
function cursorDown(n: number) { stderr.write(`\x1b[${n}B`); }
function cursorCol(n: number) { stderr.write(`\x1b[${n}G`); }
...
export async function confirm(prompt: string, defaultYes = true): Promise<boolean | null> {
